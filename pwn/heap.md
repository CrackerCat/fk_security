...
size: 该chunk的大小，大小必须是2*SIZE_SZ的整数倍(SIZE_SZ = sizeof(size_t),该字段的低三个bit对chunk的大小无影响。从高到低分别表示,NON_MAIN_ARENA(是否不属于主线程), IS_MAPPED, PREV_INUSE(记录前一个chunk是否分配,堆中第一个分配的内存块为1)。      
...
### unsorted bin  
unsorted bin 可以视为空闲 chunk 回归其所属 bin 之前的缓冲区。它只有一个链表，处于bin数组下标1处。  
使用过程中，遍历顺序是FIFO。  
